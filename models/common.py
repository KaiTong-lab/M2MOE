...
class MDM(nn.Module):
    ...
    def forward(self, x): # [batch_size, feature_num, seq_len]
        if self.layernorm:
            x = self.norm(torch.flatten(x, 1, -1)).reshape(x.shape) # [batch_size, feature_num, seq_len]
        if self.k == 0: # 3
            return x
...
class DDI(nn.Module):
    ...
    def forward(self, x):
        ...
        output = torch.zeros_like(x) # [batch_size, feature_num, seq_len]
        ...
        for i in range(self.n_history * self.patch, self.input_shape[0], self.patch):
            ...
            input = output[:, :, i - self.n_history * self.patch: i] # [batch_size, feature_num, self.n_history * patch]
            # input [batch_size, feature_num, self.n_history * patch]
            input = self.norm1(torch.flatten(input, 1, -1)).reshape(input.shape)# [batch_size, feature_num, self.n_history * patch]
            # aggregation
            # [batch_size, feature_num, patch]
            input = F.gelu(self.agg(input))  # self.n_history * patch -> patch # [batch_size, feature_num, patch]
            input = self.dropout_t(input) # [batch_size, feature_num, patch]
            # input [batch_size, feature_num, patch]
            # input = torch.squeeze(input, dim=-1)
            tmp = input + x[:, :, i: i + self.patch] # [batch_size, feature_num, patch]

            res = tmp # [batch_size, feature_num, patch]

            # [batch_size, feature_num, patch]
            if self.alpha > 0.0:
                tmp = self.norm2(torch.flatten(tmp, 1, -1)).reshape(tmp.shape) # [batch_size, feature_num, patch]
                tmp = torch.transpose(tmp, 1, 2) # [batch_size, patch, feature_num]
                # [batch_size, patch, feature_num]
                tmp = self.fc_block(tmp) # [batch_size, patch, feature_num]
                tmp = torch.transpose(tmp, 1, 2) # [batch_size, feature_num, patch]
            output[:, :, i: i + self.patch] = res + self.alpha * tmp

        # [batch_size, feature_num, seq_len]
        return output # [batch_size, feature_num, seq_len]

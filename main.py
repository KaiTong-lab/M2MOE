...
def set_seed(seed):
    ...
    torch.cuda.manual_seed_all(seed)  # 多卡训练也固定
...
from M2MOE.models.M2MoE import M2MoE 
def main(args):
    # select device
    ...
    model = M2MoE(
        ...
        patch_configs=args.patch_config,
        moe_k=args.moe_k,
...
def parse_args():
    ...
    parser.add_argument(
        ...
        '--pred_len', type=int, default=12, help='prediction sequence length'
    )
    # model hyperparameter
    parser.add_argument(
        '--moe_k', type=int, default=926, help='moe expert number'
    )
    parser.add_argument(
        '--patch_config', type=list, default=[(16, 8), (24, 12), (32, 16), (48, 24)], help='patch configuration'
    )
    parser.add_argument(

        '--loss_coef', type=float, default=0.01, help='loss coefficient for MoE loss'
    )
    parser.add_argument(
        '--norm',
        type=bool,
    ...
    parser.add_argument(
        '--batch_size', type=int, default=2, help='batch size of input data'
    )
    parser.add_argument(
        '--learning_rate',
        type=float,
